<template>
  <div class="hello">
    ...
    <p name="account" v-if="account.isConnect">address:{{account.address}}</p><br /><br />
...
import { BuyMeACoffee__factory, BuyMeACoffee } from "../types";
...
export default defineComponent({
  ...
  methods: {
    ...
    async getContract() {
      ...
      const contract = BuyMeACoffee__factory.connect(
        buyMeACoffeeAddress,
        signer
      );
      return contract;
          ...
          if (memosData.length > 0) {
            for (var memo of memosData) {
              this.memos.push({
                name: memo.name,
                from: memo.from,
                timestamp: new Date(memo.timestamp.toNumber() * 1000).toDateString(),
