...
async function main() {
	...
	console.log("current balance of owner: ", await getBalance(signer.provider!, signer.address), "ETH");
	const contractBalance = await getBalance(signer.provider!, buyCoffee.address);
	console.log("current balance of contract: ", await getBalance(signer.provider!, buyCoffee.address), "ETH");
